        ...
        match exp.node_type {
            TokenType::Aexp | TokenType::AexpMul => {
                if exp.children.len() == 0 {
                    ...
                    let mut left = Arith::create_from(&exp.children[0])?;
                        ...
                        for i in (1..exp.children.len()).step_by(2) {
                            ...
                            let right = match exp.children[i + 1].node_type {
                                TokenType::Apara | TokenType::AexpMul => {
                                    Arith::create_from(&exp.children[i+1])
                                },
                                _ => {
            ...
            TokenType::Apara => {
                if exp.data.len() > 0 {
                    ...
                    match exp.data.parse::<f32>() {
                        ...
                        Err(_) => Ok(arith::Var::new_name(exp.data.clone()))
                    }
                } else {
                    ...
                    if exp.children.len() == 0 {
                        ...
                        Arith::create_from(&exp.children[0])
                    }
                }
            }
